...
class StartViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "StartGameSegue"{
            ...
            desVC.mazeBuilder  = MazeShell(M: Int(columnCounterTextField.text!)!, N: Int(rowCounterTextField.text!)!)
        }
    }
    
    @IBAction func startGameAction(_ sender: Any) {
        if (columnCounterTextField.text?.isEmpty)!{
            Alert.action(title: "Error", message: "Enter column quantity", view: self)
        } else if (rowCounterTextField.text?.isEmpty)!{
            Alert.action(title: "Error", message: "Enter row quantity", view: self)
        } else if columnCounterTextField.text == "1" || columnCounterTextField.text == "0"{
            Alert.action(title: "Error", message: "Too few columns ", view: self)
        }else if rowCounterTextField.text == "1" || rowCounterTextField.text == "0"{
            Alert.action(title: "Error", message: "Too few rows", view: self)
        }else{
